    ...
    return (
        <Card className="w-full">
            ...
            <CardContent className="space-y-4">
                <div>
                    <Label>Edge ID</Label>
                    <Input className="mt-1 bg-muted" disabled value={selectedEdge.id} />
                </div>
                <div>
                    <Label>Source</Label>
                    <Input
...
export default function FlowDesignerPage() {
    ...
    const setNodesWithHistory = useCallback((updater: (nds: Node[]) => Node[]) => {
        pushToUndoStack();
        setNodes(updater);
    }, [pushToUndoStack, setNodes]);
    const setEdgesWithHistory = useCallback((updater: (eds: Edge[]) => Edge[]) => {
        pushToUndoStack();
        setEdges(updater);
    ...
    const onConnect = useCallback(
        (params: Connection) => setEdgesWithHistory((eds: Edge[]) => addEdge(params, eds)),
        [setEdgesWithHistory]
    );
    ...
    const deleteNode = useCallback(
        (nodeId: string) => {
            setNodesWithHistory((nds: Node[]) => nds.filter((node: Node) => node.id !== nodeId));
    ...
    const deleteEdge = useCallback(
        (edgeId: string) => {
            setEdgesWithHistory((eds: Edge[]) => eds.filter((edge: Edge) => edge.id !== edgeId));
            setSelectedEdge(null);
        },
        [setEdgesWithHistory]
    );

...
export function ChangeWorkingHoursInput() {
  ...
  const handleToggleDay = (dayId: number) => {
    setLocalWorkingHours(prev => ({
      ...prev,
      [dayId]: (prev[dayId]?.from ?? 0) > 0 || (prev[dayId]?.to ?? 0) > 0 ? { from: 0, to: 0 } : { from: 9, to: 17 },
    }));
  };
  ...
  const handleTimeChange = (dayId: number, timeType: "from" | "to", value: TimeValue | null) => {
    ...
    setLocalWorkingHours(prev => {
      const currentDay = prev[dayId] ?? { from: 0, to: 0 };
      const updatedDay = { ...currentDay, [timeType]: value.hour };
  ...
  const handleSave = () => {
    ...
    for (const dayId in updatedWorkingHours) {
      const day = updatedWorkingHours[parseInt(dayId)] ?? { from: 0, to: 0 };
      const localDay = localWorkingHours[parseInt(dayId)] ?? { from: 0, to: 0 };
      const isDayActive = localDay.from > 0 || localDay.to > 0;
    ...
    <div className="flex flex-col gap-2">
      ...
      <div className="space-y-4">
        {DAYS_OF_WEEK.map(day => {
          const dayData = localWorkingHours[day.index] ?? { from: 0, to: 0 };
          const isDayActive = dayData.from > 0 || dayData.to > 0;
                ...
                <div className="flex items-center gap-4">
                  <div className="flex items-center gap-2">
                    ...
                    <TimeInput
                      ...
                      hourCycle={12}
                      granularity="hour"
                      value={{ hour: dayData.from, minute: 0 } as TimeValue}
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <TimeInput
                      ...
                      hourCycle={12}
                      granularity="hour"
                      value={{ hour: dayData.to, minute: 0 } as TimeValue}
